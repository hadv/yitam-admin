 ...
 */
const extractInnerTubeContext = (html: string) => {
  let apiKey, clientVersion;
  ...
  if (clientVersionMatch) clientVersion = clientVersionMatch[1];
...
export const scrapeTranscriptFromYouTube = async (
  ...
  const performScrape = async (): Promise<string> => {
    try {
      ...
      if (cookieHeader) {
        console.log(`🍪 Acquired session cookies: ${cookies?.length} cookies found`);
      }

      // Extract InnerTube context
      const { apiKey, clientVersion } = extractInnerTubeContext(html);
      console.log(`🔍 Extracted InnerTube context: apiKey=${apiKey ? 'found' : 'missing'}, clientVersion=${clientVersion || 'default'}`);
      ...
      if (!transcript) {
        ...
        if (initialDataMatch && initialDataMatch[1]) {
          try {
            ...
            const extractedTranscript = await extractTranscriptFromInitialData(initialData, videoId, apiKey, clientVersion, userAgent, cookieHeader);
            if (extractedTranscript) {
              transcript = extractedTranscript;
 ...
 */
const extractTranscriptFromInitialData = async (initialData: any, videoId: string, apiKey?: string, clientVersion?: string, userAgent?: string, cookie?: string): Promise<string | null> => {
  try {
    ...
    return await fetchTranscriptWithContinuationToken(params, videoId, apiKey, clientVersion, userAgent, cookie);
  } catch (error) {
 ...
 */
const fetchTranscriptWithContinuationToken = async (params: string, videoId: string, apiKey?: string, clientVersion?: string, userAgent?: string, cookie?: string): Promise<string | null> => {
  try {
    console.log('Fetching transcript using YouTube internal API...');

    const currentClientVersion = clientVersion || '2.20241210.01.00';
    ...
    const response = await axios.post(url, {
      ...
      headers: {
        ...
        'X-YouTube-Client-Version': currentClientVersion,
