...
export const processYoutubeVideo = async (req: Request, res: Response) => {
  try {
    ...
    // Default increased to 15000 (approx 4x previous) as requested
    const chunkSize = req.body.chunkSize ? parseInt(req.body.chunkSize, 10) : 15000;
    ...
    const job = await addYoutubeProcessingJob({
      ...
      chunkSize,
      chunkOverlap,
      cookiesBrowser: req.body.cookiesFromBrowser // Use parameter name consistent with download endpoint
    });
